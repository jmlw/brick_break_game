...
public class Paddle : MonoBehaviour, IPaddle
{
    ...
    //  public CrossPlatformInputManager inputs;
    ...
    private struct PaddleSizes {
        ...
        public PaddleSizes(GameObject[] sizesArray)
            : this() {
...
//            }
//        }
        if (CrossPlatformInputManager.GetAxis("Horizontal") > 0.001 || CrossPlatformInputManager.GetAxis("Horizontal") < 0.001) {
            paddleTransform.Translate(CrossPlatformInputManager.GetAxis("Horizontal") * speed * Time.deltaTime, 
                0, 
                0);
        }
    }
    ...
    void IPaddle.Launch() {
        if (gameData.getAttachedBalls().Peek() != null) {
            gameData.getAttachedBalls().Pop().Launch();
        }
    }
